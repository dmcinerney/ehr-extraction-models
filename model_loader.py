...
model_components = {
    ...
    'code_supervision_individual_sentence': {
        'batcher_class': lambda hierarchy, counts, run_type: Batcher(hierarchy, resample_neg_proportion=.01 if run_type == 'training' else None, counts=counts, code_id=True),
        'model_class': lambda device, batcher, cluster:Model_is(sentences_per_checkpoint=p.sentences_per_checkpoint, device1=device, device2='cpu', cluster=cluster, code_embedding_type_params=batcher.get_code_embedding_type_params()),
        'optimizer_class': lambda parameters: torch.optim.Adam(parameters, lr=.001),
    ...
    'cosine_similarity': {
        ...
        'model_class': lambda device, batcher, cluster:Model_cs(sentences_per_checkpoint=p.sentences_per_checkpoint, device=device, cluster=cluster),
    ...
    'distance': {
        'batcher_class': lambda hierarchy, counts, run_type: Batcher(hierarchy, code_description=True, add_special_tokens=False),
        'model_class': lambda device, batcher, cluster:Model_d(sentences_per_checkpoint=p.sentences_per_checkpoint, device=device, cluster=cluster),
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.hierarchy, batcher.code_idxs, OBT_cs if run_type == 'testing' else OBA_cs)},
    'tfidf_similarity': {
