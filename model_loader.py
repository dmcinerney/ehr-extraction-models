...
model_components = {
    'code_supervision': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, resample_neg_proportion=.01 if run_type == 'training' else None, counts=counts, code_id=True),
    ...
    'code_supervision_only_linearization': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, resample_neg_proportion=.01 if run_type == 'training' else None, counts=counts, code_linearization=True),
    ...
    'code_supervision_only_description': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, resample_neg_proportion=.01 if run_type == 'training' else None, counts=counts, code_description=True),
        'model_class': lambda device, batcher:Model(sentences_per_checkpoint=p.sentences_per_checkpoint, num_codes=len(batcher.code_idxs), device1=device, device2='cpu', freeze_bert=True),
        'optimizer_class': lambda parameters: torch.optim.Adam(parameters, lr=.001),
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.graph_ops, batcher.code_idxs, (OB if run_type == 'training' else (OBT if run_type == 'testing' else OBA)))},
    ...
    'code_supervision_only_linearization_description_unfrozen': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, resample_neg_proportion=.01 if run_type == 'training' else None, counts=counts, description_linearization=True),
        'model_class': lambda device, batcher:Model(sentences_per_checkpoint=p.sentences_per_checkpoint, num_codes=len(batcher.code_idxs), device1=device, device2='cpu', freeze_bert=False, dropout=0),
        'optimizer_class': lambda parameters: torch.optim.Adam(parameters, lr=.00001),
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.graph_ops, batcher.code_idxs, (OB if run_type == 'training' else (OBT if run_type == 'testing' else OBA)))},
    ...
    'code_supervision_individual_sentence_unfrozen': {
        ...
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.graph_ops, batcher.code_idxs, (OB_is if run_type == 'training' else (OBT_is if run_type == 'testing' else OBA_is)))},
    'cosine_similarity': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, code_description=True, add_special_tokens=False),
    ...
    'distance': {
        ...
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.graph_ops, batcher.code_idxs, OBA_cs)},
    'tfidf_similarity': {
        'batcher_class': lambda code_graph, counts, run_type: Batcher(code_graph, code_description=True, tfidf_tokenizer=True),
        'model_class': lambda device, batcher: Model_tfidf(device=device),
        'postprocessor': lambda batcher, run_type: Postprocessor(batcher.graph_ops, batcher.code_idxs, OBA_cs)},
}


def load_model_components(model_name, code_graph_file, run_type='training', device='cpu', model_file=None, optimizer_file=None, counts_file=None):
    counts = None if counts_file is None else read_pickle(counts_file)
    batcher = model_components[model_name]['batcher_class'](read_pickle(code_graph_file), counts, run_type)
