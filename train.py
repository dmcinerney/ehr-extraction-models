...
#from dataset_scripts.evidence_inference.batcher import Batcher, SentenceBatcher, EvidenceBatcher
from dataset_scripts.pubmed.batcher import PubmedBatcher
from models.clinical_bert.model import ClinicalBertExtractionModel, loss_func, statistics_func
...
def main(checkpoint_folder=None):
    ...
    batch_iterator = batcher.batch_iterator(train_dataset, indices_iterator, subbatches=32, devices=device)
    val_iterator = batcher.batch_iterator(val_dataset, val_indices_iterator, subbatches=32, devices=device)
    model = ClinicalBertExtractionModel().to(device)
    optimizer = get_optimizer(list(model.named_parameters()), len(indices_iterator))
    trainer = Trainer(model, optimizer, batch_iterator, checkpoint_folder='checkpoints/clinical_bert_pubmed/checkpoint', checkpoint_every=10, val_iterator=val_iterator, val_every=10)
