...
from models.ehr_extraction.code_supervision.iteration_info import get_batch_info_test_class
...
BatchInfoTest = get_batch_info_test_class(loss_func)
...
def main(load_checkpoint_folder=None):
    ...
    model = Model(len(batcher.code_graph.nodes), sentences_per_checkpoint=17, device1=device1, device2=device2, freeze_bert=True, reduce_code_embeddings=True)
...
#    tester = Tester(model, val_iterator, batch_info_class=BatchInfoTest)
    tester = Tester(model, val_iterator, batch_info_class=BatchInfoTest, tensorboard_dir=os.path.join(load_checkpoint_folder, 'tensorboard/test_time2'))
    tester.test()
